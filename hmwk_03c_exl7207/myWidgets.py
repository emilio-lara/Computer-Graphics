...
class cl_toolbar :
  ...
  def distance_callback(self):
    theOldValue=0.0
    someMinVal = 0
    f = simpledialog.askfloat( "Distance", "Enter new distance: ",
                               initialvalue = theOldValue, minvalue = someMinVal )
    if( f!= None):
      self.master.distance= f
      self.master.statusBar_frame.set( " Distance is now: %.1f",f )
  ...
  def draw_callback( self ) :
    if self.master.m_ModelData is None :
      ...
      return
    clip = self.master.Clip.get()
    perpective=self.master.Perspective.get()
    d = self.master.distance
    print(clip)
    print(perpective)
    print(d)
    ...
    self.master.m_ModelData.specifyTransform( ax, ay, sx, sy,d,center )
    ...
    self.master.ob_world.create_graphic_objects( self.master.ob_canvas_frame.canvas,
                                                 self.master.m_ModelData,clip,perpective)
