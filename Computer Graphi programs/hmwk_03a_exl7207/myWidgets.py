...
class cl_menu :
  def __init__( self, master ) :
    ...
    dummy.add_checkbutton(label = 'Clipping', variable=self.master.var)
...
class cl_toolbar :
  ...
  def load_callback( self ) :
    ...
    self.fName = tk.filedialog.askopenfilename( filetypes = [ ("allfiles","*")])
    if len(self.fName)==0:
      self.master.statusBar_frame.set( 'cancel load' )
    else:
      self.master.statusBar_frame.set( 'Load file' )
